...
customElements.define("webaudio-pianoroll", class Pianoroll extends HTMLElement {
    ...
    oscillator.type = "sawtooth"; // You can change this to other types like 'sine', 'square', or 'triangle'
    oscillator.frequency.value = this.midiToFrequency(ev.n); // Adjust frequency as needed

    // Connect oscillator to gain node
    oscillator.connect(gainNode);

    // Set gain value and connect to output
    gainNode.gain.value = 0; // Adjust volume as needed
    ...
    setTimeout(() => {
        ...
        oscillator.stop();
    }, 450); // 0.5 seconds in milliseconds
}
...
<div id="wac-menu">Delete</div>
        ...
        this.moveSelectedNote=function(dt,dn){
            // console.log("Moved Note:" + dn) // My Code
            // let oldNoteMove = this.lastNoteMove
            // this.lastNoteMove = dn
            //
            // if(dn !== oldNoteMove){
            //     let ev = this.selectedNotes()[0]['ev']
            //     var n = ev.n += dn
            //     let newev = {n:n, t:ev.t, g:ev.g, f:ev.f}
            //     this.playNoteForClick(newev)
            // }
        ...
        this.editDragDown=function(pos){
            ...
            if(ht.m=="N"){
                ...
                //start of my code
                const pressedNote = this.selectedNotes()[0]['ev']
                console.log(pressedNote)
                this.playNoteForClick(pressedNote)
